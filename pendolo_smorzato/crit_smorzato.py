...
corrente, lambda1 = np.loadtxt("./lambda.csv", delimiter=";", unpack=True, skiprows=1)
...
ax.errorbar(corrente, lambda1, ls="none", marker="o", mfc="b", ms=5, mec="b")
...
parametri, covar = optimize.curve_fit(f, corrente, lambda1)
print(f"A = {parametri[0]}\nB = {parametri[1]}\nC = {parametri[2]}")
# grafico con tendenza
fig2, ax2 = mp.subplots()
ax2.set_title(r"relazione I - $\lambda$", fontsize=16, pad=15)
ax2.set_xlabel("CORRENTE (A)", fontsize=11)
ax2.set_ylabel("LAMBDA", fontsize=11)
...
ax2.text(
    0.1,
    1,
    rf"$\lambda$ = ${A1:.2f}x^2 {B1:+.2f}x {C1:+.2f}$",
    fontsize=11,

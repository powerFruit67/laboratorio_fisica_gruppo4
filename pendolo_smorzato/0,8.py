...
# grafico dei punti
tempo, tetaP = np.loadtxt("./0,8.csv", delimiter=",", unpack=True, skiprows=1)
teta = tetaP / 180 * np.pi
fig, ax = mp.subplots()
ax.set_xlabel("TEMPO (s)", fontsize=11)
ax.set_ylabel("TETA (rad)", fontsize=11)
ticklist = np.arange(0, 6, 1)
...
ax2.set_title("corrente = 0.8A", fontsize=16, pad=15)

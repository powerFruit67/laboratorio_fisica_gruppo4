...
# grafico dei punti
tempo, tetaP = np.loadtxt("./0,6.csv", delimiter=",", unpack=True, skiprows=1)
teta = tetaP / 180 * np.pi
fig, ax = mp.subplots()
ax.set_xlabel("TEMPO (s)", fontsize=11)
ax.set_ylabel("TETA (rad)", fontsize=11)
ticklist = np.arange(0, 10, 2)
ticklabel = ["0", "2", "4", "6", "8"]
...
ax2.set_title("corrente = 0.6A", fontsize=16, pad=16)
